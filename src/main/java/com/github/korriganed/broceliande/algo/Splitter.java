...
import com.github.korriganed.broceliande.struct.DataSet;
import com.github.korriganed.broceliande.struct.Occurrences;
...
public class Splitter<D, R> {
	...
	public BestSplit<D> findBestSplit(DataSet<D, R> dataSet, Method featureGetter) {
		...
		if (InspectionUtils.getFeatureType(featureGetter).equals(FeatureType.CATEGORICAL)) {
			...
			Stream<List<D>> sampleGroupByFeatureValue = sample.stream().map(data -> InspectionUtils.invokeGetter(data, featureGetter))
					.distinct()
					.map(featureValue -> sample.stream()
							.filter(data -> InspectionUtils.invokeGetter(data, featureGetter).equals(featureValue))
			...
			bestSplitOfFeature = sampleGroupByFeatureValue.map(group -> {
				...
				D d = group.get(0);
				Predicate<D> p = toCheck -> InspectionUtils.invokeGetter(toCheck, featureGetter).equals(InspectionUtils.invokeGetter(d, featureGetter));
